...
% performs a robust Spearman correlation on data cleaned up for 
bivariate outliers,
% that is after finding the central point in the distribution using the 
mid covariance
% determinant, orthogonal distances are computed to this point, and any 
data outside the
% bound defined by the idealf estimator of the interquartile range is 
removed.
%
% FORMAT: 
[rp,tp,CI,pval,outid,h]=skipped_Spearman(X,pairs,method,alphav,p_alpha);
%
% INPUTS:  X is a matrix and corelations between all pairs (default) are 
computed
...
%          CI is the robust confidence interval of r computed by 
bootstrapping
...
% - Verboven, S., Hubert, M. (2005), LIBRA: a MATLAB Library for Robust 
Analysis,
...
if strcmp(method,'Hochberg') && n<60 || strcmp(method,'Hochberg') && 
n<60 && alphav == 5/100
...
(sum(detrend(xrank,'constant').^2).*sum(detrend(yrank,'constant').^2)).^(1/2);
     ...
     if nargout > 2
         ...
         % fprintf('computing p values by bootstrapping data, pair %g 
%g\n',pairs(row,1),pairs(row,2))
         parfor b=1:nboot
             Xb = X(boostrap_sampling(:,b),:);
             xrank = tiedrank(Xb(keep,1),0); yrank = tiedrank(Xb(keep,2),0);
             r(b) = 
sum(detrend(xrank,'constant').*detrend(yrank,'constant')) ./ ...
(sum(detrend(xrank,'constant').^2).*sum(detrend(yrank,'constant').^2)).^(1/2);
         end
...
%% once we have all the r and t values, we need to adjust for multiple 
comparisons
if nargout == 6
     if strcmp(method,'ECP')
         if exist('p_alpha','var')
             ...
             disp('ECP method requested, computing p alpha ... (takes a 
while)')
